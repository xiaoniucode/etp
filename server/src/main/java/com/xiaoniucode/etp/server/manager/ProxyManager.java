...
public class ProxyManager {
    ...
    public ProxyConfig addProxy(String clientId, ProxyConfig proxyConfig, Consumer<ProxyConfig> callback) {
        ...
        if (ProtocolType.isHttp(protocol)) {
            ...
            proxyConfig.getDomainInfo().getFullDomains().clear();
            proxyConfig.getDomainInfo().getFullDomains().addAll(domains);
    ...
    public ValidInfo validProxy(String clientId, ProxyConfig config) {
        ...
        if (existConfig != null) {
            ...
            //如果是HTTP协议 检查域名
            if (ProtocolType.isHttp(protocol) && !config.getDomainInfo().getAutoDomain()) {
