          ...
          <ElTableColumn :label="t('common.actions')" width="240" fixed="right">
            <template #default="scope">
              <ElSpace size="small">
                ...
                <ElButton v-else link size="small" @click="handleEditRule(scope.row)">
                  {{ t('common.edit') }}
                </ElButton>
                <ElButton link size="small" @click="handleDeleteRule(scope.row.id)">
                  <template #icon>
                    <Delete/>
                  </template>
                  {{ t('common.delete') }}
...
type AccessRule = (typeof formData.rules)[number]
const editingRuleBackup = ref<AccessRule | null>(null)
