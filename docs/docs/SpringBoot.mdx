...
`etp` 支持将客户端内嵌到 `Spring Boot` 应用中运行，只需引入 `Maven/Gradle` 依赖并配置，即可实现**一键穿透**，非常适用于临时调试、开发环境访问等场景。
...
最新版本可从 [Maven 中央仓库](https://central.sonatype.com/) 中获取，目前支持 `Spring Boot 2.x/3.x/4.x`。
