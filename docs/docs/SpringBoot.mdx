...
# SpringBoot
`etp`支持客户端嵌入到`SpringBoot`应用中运行，只需引入`Maven/Gradle`坐标，添加几行配置即可无缝集成，无需单独安装`etpc`客户端，主要用于临时测试，
配置信息不会持久化到数据库。环境要求如下：

- JDK8+
- Maven or Gradle

## 引入依赖
最新版本可从[Maven中央仓库](https://central.sonatype.com/)中获取。仓库也提供了使用案例，[下载地址](https://github.com/xiaoniucode/etp)。
